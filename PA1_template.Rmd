...
intmean <- ddply(activity, "interval", summarise,
                perint = mean(steps, na.rm = TRUE))  # computes the average number of steps for each 5 minute interval
...
```{r}
missing <- sum(is.na(activity[,1])) #finds the total number of NA values
missing
